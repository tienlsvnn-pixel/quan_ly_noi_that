...
class DonHangController extends BoDieuKhien
{
    ...
    {
        $customers = KhachHang::orderBy('name')->get();
        $products = SanPham::where('is_active', true)->orderBy('name')->get();

        return view('quan_tri.don_hang.tao', compact('customers', 'products'));
    }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'status' => ['required', Rule::in(DonHang::STATUSES)],
        ...
        $order = DB::transaction(function () use ($data) {
            ...
            if ($order->isCompleted()) {
                $this->applyDonHangStock($order->fresh('items.product'));
            }
    ...
    public function show(DonHang $order): View
    {
        $order->load(['customer', 'items.product']);

        return view('quan_tri.don_hang.chi_tiet', compact('order'));
    }

    public function update(Request $request, DonHang $order): RedirectResponse
    {
        $data = $request->validate([
            'status' => ['required', Rule::in(DonHang::STATUSES)],
            'note' => ['nullable', 'string'],
        ...
        DB::transaction(function () use ($data, $order) {
            ...
            if ($oldStatus !== DonHang::STATUS_COMPLETED && $order->isCompleted()) {
                $this->applyDonHangStock($order);
            }

            if ($oldStatus === DonHang::STATUS_COMPLETED && !$order->isCompleted()) {
                $this->restoreDonHangStock($order);
            }
        });
    ...
    private function applyDonHangStock(DonHang $order): void
    {
    ...
    private function restoreDonHangStock(DonHang $order): void
    {
        if (!$order->stock_applied) {
            return;
        }
